...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
        ...
        # Credit:   Incorporates Joe Lencioni (August 6, 2008) code
    {
        ...
        if (version_compare(PHP_VERSION, '5.1.0') >= 0) {
            ...
            } else # More subtle and personally more desirable
            {
                ...
                $sharpenMatrix = array(array(-1, -2, -1), array(-2, $sharpness + 12, -2), //Lessen the effect of a filter by increasing the value in the center cell
                    array(-1, -2, -1));
    ...
    private function sharpen2($level) {
        $sharpenMatrix = array(array($level, $level, $level), array($level, (8 * $level) + 1, $level), //Lessen the effect of a filter by increasing the value in the center cell
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
        ...
        #
    {
        $len = strlen($val);
        if ($len < 0x8000)
            return chr(0x1c) . chr($rec) . chr($dat) . chr($len >> 8) . chr($len & 0xff) . $val; else
        ...
        while ($Y >= 0) {
            ...
            while ($X < $BMP['width']) {
                if ($BMP['bits_per_pixel'] == 24)
                    $COLOR = unpack("V", substr($IMG, $P, 3) . $VIDE); else if ($BMP['bits_per_pixel'] == 16) {
           ...
           */
                    ...
                    $COLOR = unpack("n", $VIDE . substr($IMG, floor($P), 1));
                    if (($P * 8) % 8 == 0)
                        $COLOR[1] = $COLOR[1] >> 7; else if (($P * 8) % 8 == 1)
        ...
        #
    {
        if (file_exists($this->psdReaderPath)) {
            ...
            if (isset($psdReader->infoArray['error']))
                return ''; else return $psdReader->getImage();
        } else {
            return false;
        }
    }
